...
export default function Items() {
  ...
  const uploadImage = async (currentImageUrl: string = ''): Promise<string> => {
    ...
    try {
      // 1. Compress the image significantly to avoid timeouts on slow connections
      const options = {
        maxSizeMB: 0.1, // 100KB is very small and fast to upload
      ...
      // Return the current image URL or empty so the save process can continue
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => {
                      if (isAdding) setNewItem({...newItem, purchasePrice: e.target.value});
                      else if (editingItem) setEditingItem({...editingItem, purchasePrice: e.target.value as any});
                    }}
                    className="w-full bg-background border border-gray-200 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-secondary/50"
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => {
                      if (isAdding) setNewItem({...newItem, price: e.target.value});
                      else if (editingItem) setEditingItem({...editingItem, price: e.target.value as any});
                    }}
                    className="w-full bg-background border border-gray-200 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-secondary/50"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => {
                      if (isAdding) setNewItem({...newItem, stock: e.target.value});
                      else if (editingItem) setEditingItem({...editingItem, stock: e.target.value as any});
                    }}
                    className="w-full bg-background border border-gray-200 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-secondary/50"
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => {
                      if (isAdding) setNewItem({...newItem, minStock: e.target.value});
                      else if (editingItem) setEditingItem({...editingItem, minStock: e.target.value as any});
                    }}
                    className="w-full bg-background border border-gray-200 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-secondary/50"
                  />
                </div>
          ...
          <table className="w-full text-right">
            <thead className="bg-background border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-bold text-secondary uppercase tracking-wider">السعر</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center text-gray-400">جاري تحميل البيانات...</td>
                </tr>
              ) : filteredItems.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center text-gray-400">لا توجد أصناف تطابق البحث</td>
                </tr>
                ...
                <tr key={item.id} className={`hover:bg-background/50 transition-colors ${selectedItems.includes(item.id) ? 'bg-secondary/5' : ''}`}>
                  ...
                  <td className="px-6 py-4">
                    <p className="text-sm font-bold text-primary">{item.price.toLocaleString()} {shopSettings?.currency || 'ر.ي'}</p>
                    <p className="text-[10px] text-gray-400">للوحدة ({item.unit})</p>
                  </td>
                  <td className="px-6 py-4">
        ...
        {selectedItems.length > 0 && (
          ...
          >
            ...
            <div className="w-full md:flex-1 flex gap-2 md:gap-3">
              ...
              <button 
                onClick={() => {}}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 md:py-3 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl text-xs sm:text-sm font-bold transition-all opacity-50 cursor-not-allowed"
                title="سيتم دعم الحذف الجماعي قريباً"
              >
